...
class DualRowSignalHookV : StatusBarViewUtils() {
    ...
    override fun onDarkModeChanged(ctx: StatusBarViewContext, darkInfo: DarkInfo) {
        ...
        val dualContainer = ctx.findViewWithTag<FrameLayout>(TAG_DUAL_CONTAINER) ?: return
        ...
        val (slot1ResId, slot2ResId) = getDualSignalIconPairResId(
            darkInfo.isUseTint, darkInfo.isLight
        )
        ...
        if (slot1ResId != null && slot2ResId != null) {
            slot1.post {
                ...
                val tintList = darkInfo.color?.let { color ->
                    if (darkInfo.isUseTint) ColorStateList.valueOf(color) else null
                } ?: slot1.imageTintList
        ...
        mobileSignalController.methodFinder()
            ...
            .createHook {
                after { param ->
                    ...
                    val level = if (rawLevel >= 4 && isMoreHyperOSVersion(3f)) 5 else rawLevel
    ...
    private fun getDualSignalIconPairResId(
        ...
        val (dataLevel, noDataLevel) = getSignalLevelsForRender()
        return Pair(
            dualSignalResMap[getSignalIconResName(1, dataLevel, isUseTint, isLight)],
            dualSignalResMap[getSignalIconResName(2, noDataLevel, isUseTint, isLight)]
        )
    }
}
