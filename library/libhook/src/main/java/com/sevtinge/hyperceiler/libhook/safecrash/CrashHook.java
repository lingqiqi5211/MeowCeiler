...
import static com.sevtinge.hyperceiler.libhook.utils.hookapi.tool.EzxHelpUtils.hookAllMethods;
...
public class CrashHook {
    private static final String TAG = "CrashHook";
    ...
    public CrashHook(XposedModuleInterface.SystemServerLoadedParam loadPackageParam) throws Exception {
        backgroundActivity(loadPackageParam.getClassLoader());
        init(loadPackageParam.getClassLoader());
    ...
    public void init(ClassLoader classLoader) throws Exception {
        ...
        hookMethod(hookError, new IMethodHook() {
            @Override
            public void after(AfterHookParam param) {
                Context mContext = (Context) EzxHelpUtils.getObjectField(param.getThisObject(), "mContext");
                Object proc = param.getArgs()[0];
                ApplicationErrorReport.CrashInfo crashInfo = (ApplicationErrorReport.CrashInfo) param.getArgs()[1];
                String shortMsg = (String) param.getArgs()[2];
                String longMsg = (String) param.getArgs()[3];
                String stackTrace = (String) param.getArgs()[4];
                long timeMillis = (long) param.getArgs()[5];
                int callingPid = (int) param.getArgs()[6];
                int callingUid = (int) param.getArgs()[7];
                XposedLog.e("CrashHook", "context: " + mContext + " pkg: " + mContext.getPackageName() + " proc: " + proc + " crash: " + crashInfo + " short: " + shortMsg
            ...
            findAndHookMethod("com.android.server.wm.BackgroundActivityStartController", classLoader, "checkBackgroundActivityStart",
                ...
                new IMethodHook() {
                    ...
                    public void before(BeforeHookParam param) {
                        String pkg = (String) param.getArgs()[2];
                        if (pkg == null) return;
                        if (ProjectApi.mAppModulePkg.equals(pkg)) {
                            Object balAllowDefault = EzxHelpUtils.getStaticObjectField(
                                findClass("com.android.server.wm.BackgroundActivityStartController$BalVerdict", classLoader),
                                "BAL_ALLOW_DEFAULT"
                            );
        ...
        hookAllMethods("com.android.server.wm.ActivityStarterImpl", classLoader,
            "isAllowedStartActivity",
                new IMethodHook() {
                    ...
                    public void before(BeforeHookParam param) {
                        int count = -1;
                        for (Object clz : param.getArgs()) {
                            count = count + 1;
                            if (clz instanceof String) {
                                break;
                            }
                        }
                        String pkg = (String) param.getArgs()[count];
                        if (pkg == null) return;
                        if (ProjectApi.mAppModulePkg.equals(pkg)) {
    ...
    private void recordCrash(Context mContext, Object proc, ApplicationErrorReport.CrashInfo crashInfo,
        ...
        ApplicationInfo info = (ApplicationInfo) EzxHelpUtils.getObjectField(proc, "info");
    ...
    private void reportCrashByIntent(ArrayList<JSONObject> report) {
        ...
        for (JSONObject j : report) {
            ...
            if (stringBuilder.isEmpty()) stringBuilder.append(b);
            else stringBuilder.append(",").append(b);
        }

        String abbr = scopeMap.get(CrashRecord.getPkg(report.getFirst()));
