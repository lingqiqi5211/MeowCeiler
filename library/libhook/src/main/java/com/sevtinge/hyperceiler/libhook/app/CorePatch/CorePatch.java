...
public class CorePatch {
    ...
    public void onLoad(XposedModuleInterface.SystemServerLoadedParam lpparam) {
        ...
        if (!ProjectApi.isRelease()) {
            XposedLog.i(TAG, "system", "downgrade=" + mPrefsMap.getBoolean("system_framework_core_patch_downgr", true));
            XposedLog.i(TAG, "system", "authcreak=" + mPrefsMap.getBoolean("system_framework_core_patch_auth_creak", true));
            XposedLog.i(TAG, "system", "digestCreak=" + mPrefsMap.getBoolean("system_framework_core_patch_digest_creak", true));
            XposedLog.i(TAG, "system", "UsePreSig=" + mPrefsMap.getBoolean("system_framework_core_patch_use_pre_signature", false));
            XposedLog.i(TAG, "system", "exactSignatureCheck=" + mPrefsMap.getBoolean("system_framework_core_patch_exact_signature_check", false));
            XposedLog.i(TAG, "system", "sharedUser=" + mPrefsMap.getBoolean("system_framework_core_patch_shared_user", false));
            XposedLog.i(TAG, "system", "disableVerificationAgent=" + mPrefsMap.getBoolean("system_framework_disable_verification_agent", true));
        }
