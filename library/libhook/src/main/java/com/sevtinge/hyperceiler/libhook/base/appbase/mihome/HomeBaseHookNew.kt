...
*/
package com.sevtinge.hyperceiler.libhook.base.appbase.mihome
        ...
        for (m in methods) {
            ...
            if (anno != null) {
                ...
                if (version >= anno.min && version <= anno.max && (!isPadSpecified || anno.isPad == isPadCached)) {
                    try {
                        XposedLog.d(TAG, lpparam.packageName, "Check method ${m.name} for version $version, select ${anno.min} to ${anno.max}, isPad = ${anno.isPad}")
                        m.isAccessible = true
                        m.invoke(this)
                        return
                    } catch (t: Throwable) {
