...
@Composable
fun SystemUiClockPage(onBack: () -> Unit, onOpen: (Route) -> Unit) {
    ...
    HostPage(
        titleRes = R.string.systemui_clock,
        hostPackage = Scope.SystemUi,
        onBack = onBack,
    ) {
        SettingsCard(testTag = "section.systemui.clock") {
            FeatureSwitchRow(key = Preferences.SystemUi.Clock, titleRes = R.string.preference_enabled)
            FeatureSwitchRow(
        ...
        SettingsSection(titleRes = R.string.systemui_clock_section_layout, testTag = "section.systemui.clock.layout") {
            ...
            if (isPadDevice) {
                SettingsNavigationRow(titleRes = R.string.systemui_clock_section_pad, testTag = "row.systemui.clock.layout.pad") {
                    onOpen(Route.SystemUiClockLayout(ClockPart.Pad))
                }
            }
        }
    }
...
fun SystemUiClockLayoutPage(part: ClockPart, onBack: () -> Unit) {
    ...
    val titleRes = when (part) {
        ...
        ClockPart.Pad -> R.string.systemui_clock_section_pad
    }
    val padHidden by rememberMeowPreferenceValue(p.ClockHiddenPad)
    HostPage(titleRes = titleRes, hostPackage = Scope.SystemUi, onBack = onBack) {
        if (part == ClockPart.Pad && !isPadDevice) return@HostPage
        if (part == ClockPart.Pad) {
            SettingsCard(testTag = "section.systemui.clock.pad.hidden") {
                FeatureSwitchRow(
                    key = p.ClockHiddenPad,
                    titleRes = R.string.systemui_clock_hidden,
                    summaryRes = R.string.systemui_clock_hidden_summary,
                )
            }
            if (padHidden) return@HostPage
            SettingsCard(testTag = "section.systemui.clock.pad.format") {
                FeatureTextRow(p.ClockFormatPad, R.string.systemui_clock_format_pad, allowBlank = true)
                SettingsNavigationRow(titleRes = R.string.systemui_clock_format_help, testTag = "row.systemui.clock.pad.help") {
                    context.startActivity(Intent(Intent.ACTION_VIEW, Uri.parse(FormatHelpUrl)))
                }
            }
        }
        SettingsCard(testTag = "section.systemui.clock.layout.${part.name}") {
            when (part) {
                ...
                ClockPart.Pad -> {
                    FeatureSwitchRow(key = p.ClockBoldPad, titleRes = R.string.systemui_clock_bold)
                    FeatureSliderRow(p.ClockSizePad, R.string.systemui_clock_size, 0f..72f, 1f, "sp")
                }
            }
        }
        SettingsCard(testTag = "section.systemui.clock.layout.${part.name}.sliders") {
            when (part) {
                ...
                ClockPart.Pad -> {
                    FeatureSliderRow(p.ClockLeftPad, R.string.systemui_clock_left, 0f..72f, step = 0.5f, unit = "dp")
                    FeatureSliderRow(p.ClockRightPad, R.string.systemui_clock_right, 0f..72f, step = 0.5f, unit = "dp")
                    FeatureSliderRow(p.ClockOffsetPad, R.string.systemui_clock_offset, 0f..24f, step = 0.5f, unit = "dp")
                }
            }
        }
    }
